...
export default function ServerBlock({ setIsPressed }) {
  ...
  return (
    ...
    >
      ...
      onClick={() => navigate("/server/1")} onMouseDown={() => setIsPressedMouse(true)} onMouseUp={() => setIsPressedMouse(false)} onMouseLeave={() => setIsPressedMouse(false)} className={`cursor-pointer w-full py-3 px-3.5 bg-[#2A2C2B] rounded-2xl flex flex-col gap-3.5 duration-150 ${isPressedMouse ? "scale-90" : ""}`}>
